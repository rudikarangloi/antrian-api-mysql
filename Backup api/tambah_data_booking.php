...
//if($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	//Jika antrian ke loket pendaftaran
	if($jenis_antrian_poliklinik == "0"){				
		...
		$filter_jenis_antrian = " AND jenis_antrian_poliklinik <> '0' ";
		//$query = " SELECT count(*) as count FROM data_antrian WHERE counter='". $loket."' ". $filter_waktu;	
		$query = " SELECT count(*) as count FROM data_antrian WHERE counter='". $loket."' ". $filter_waktu . $filter_jenis_antrian;	
		$queries = " SELECT count(*) as count FROM data_antrian WHERE counter='". $loket."' ". $filter_waktu . $filter_jenis_antrian;			
	}
	...
	if($jmlPasien >= $jmlKouta) {
		...
		*/
		...
		$waktu_booking_full = $waktu_booking . " 10:11:08";
		$sql = "INSERT INTO data_antrian SET 
	            counter        = '$loket',
	            waktu          = '$waktu_booking_full',
	            status         = '$status',
		...
		if(mysqli_query($conn,$sql)){
			//echo $jmlCountId;
			echo $jmlCountId.'***'.$KodeBookings.'***'.$queries;
		}else{
